...
const CaptainEarnings: React.FC<CaptainEarningsProps> = ({ captainId }) => {
    ...
    const fetchEarnings = async () => {
      try {
        ...
        const { data: totalData, error: totalError } = await supabase
          .from('deliveries')
          .select('delivery_fee, created_at') // Make sure to include created_at in the query
        ...
        const today = totalData?.filter(delivery => {
          if (!delivery.created_at) return false; // Check if created_at exists
